...
def process_text():
    ...
    get_sub_locations_api()
    # Return the processed class and description
    return jsonify({'Class': output_text, 'Description': desc, 'SubLocations': sub_locations})

#AIzaSyDUVPf98MB2xHl8___C7gXhTpPjcoisE0Q
# Helper function to extract sub-locations
gmaps = googlemaps.Client(key="AIzaSyDUVPf98MB2xHl8___C7gXhTpPjcoisE0Q")

def get_sub_locations(location_name):
    """
    Convert a location name into geographical coordinates using Google Maps API.
...
@app.route('/get_sub_locations', methods=['GET'])
def get_sub_locations_api():
    """
    Fetch all locations from the database and return them as JSON.

    :return: JSON response containing all locations.
    """
